"""Small utility to validate MongoDB connection from environment variables.
Run: python3 scripts/check_mongo.py

Supports both direct MONGO_DB_URI and Railway environment variables:
- MONGO_DB_URI (preferred)
- MONGO_URL (Railway)
- Individual components: MONGOUSER, MONGOPASSWORD, MONGOHOST, MONGOPORT
"""
...
# Try to construct MONGO_DB_URI from various sources
def get_mongo_uri():
    # Try direct MONGO_DB_URI first
    if getenv('MONGO_DB_URI'):
        return getenv('MONGO_DB_URI'), 'MONGO_DB_URI'
    
    # Try Railway MONGO_URL
    if getenv('MONGO_URL'):
        return getenv('MONGO_URL'), 'MONGO_URL'
    
    # Try to construct from individual variables (Railway pattern)
